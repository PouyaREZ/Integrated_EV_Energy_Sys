...
Percent_EV_Load_On_CCHP = 0 # Percentage of EV charging load carried by the CCHP set as a constant; only considered when Optimize_EV_Load_On_CCHP == False
...
Building_Min = 0
Supply_Min = 1
Vars_Plus_Output = 45  # Must be updated if new chillers, engines, or outputs are added; Must also update optimization initialization
...
EV_Input = np.genfromtxt('Hourly_EV.csv', float, delimiter=',', skip_header=1)
...
def SupplyandDemandOptimization(Building_Var_Inputs):    
    ...
    if Optimize_EV_Load_On_CCHP == True:
        EV_Load_Ratio = Building_Var_Inputs[Num_Buildings+2]
    else:
...
#    Comm_Solar_Type_Var = 1 ## ?? WHAT IS IT? Only the first solar type is used! NOT OPTIMIZING FOR SOLAR PANEL TYPE
    ...
    Total_EV_Demand, EV_Charging_GHG, EV_Charging_LCC, Peak_EV_Demand =\
...
#    GW_Efficiency = np.nan_to_num(Total_Treated_GW/Total_GW) ## Added the nan_to_number
    ...
    Run_Result[0][Num_Buildings] = Engine_Var # i.e. element 21
    Run_Result[0][Num_Buildings+1] = Chiller_Var # i.e. element 22
    Run_Result[0][Num_Buildings+2] = Total_Electricity_Demand # Electric demand from buildings # i.e. element 23
    Run_Result[0][Num_Buildings+3] = EV_Load_Ratio # i.e. element 24
    # Now the objectives
