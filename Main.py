'''
Unit of heat and electricity: kWh (aka kW)
'''
...
from EV import EV_Load_Calculator
...
Current_Year = 2021
midyear = round(Project_Life/2+Current_Year)


USD_2008_to_2021 = 1.21             # From https://www.in2013dollars.com/2008-dollars-in-2021
USD_2007_to_2021 = 1.26             # From https://www.in2013dollars.com/2007-dollars-in-2021


## Annual electricity consumption of SF county from linear interpolation on data from 2012-2019 sourced from http://ecdms.energy.ca.gov/elecbycounty.aspx
# NonResidential_Electricity_Use_SF = 3900.475 * GWh_to_kWh # in kWh # year: 2025
NonResidential_Electricity_Use_SF = lambda year: GWh_to_kWh * (-41.341*year + 87616) # in kWh
# Residential_Electricity_Use_SF = 1448.25 * GWh_to_kWh # in kWh # year: 2025
Residential_Electricity_Use_SF = lambda year: GWh_to_kWh * (-2.2304*year + 5964.6) # in kWh 
# Total_Electricity_Use_SF = 5348.725 * GWh_to_kWh # in kWh # year: 2025
Total_Electricity_Use_SF = lambda year: GWh_to_kWh * (-43.571*year + 93580) # in kWh

PEV_Count_By_Year = lambda year: 2795.25 * year -5632159.3  # Linearly interpolated between values for 2017 and 2025 from https://www.nrel.gov/docs/fy18osti/70893.pdf for 2017 and CEC EVI-PRO Map for 2025


NonResidential_Electricity_Use_SF = NonResidential_Electricity_Use_SF(midyear)
Residential_Electricity_Use_SF = Residential_Electricity_Use_SF(midyear)
Total_Electricity_Use_SF = Total_Electricity_Use_SF(midyear)
PEV_Count_Scale = PEV_Count_By_Year(midyear)/PEV_Count_By_Year(2025) # Coefficient to scale the PEV demand relative to year 2025 for which we have the load profiles; 2025 is the year we have the EV profiles for
...
'''-----------------------------------------------------------------------------------------------'''
Population_Size = 256 #288 ## MODIFIED -- Must be a multiple of 4
Mutation_Probability = 0.05
Crossover_Probability = 0.75
Eta = 2.5
Number_Generations = 1024 #1000 ## MODIFIED


CWWTP_Mode = 0          # If 1: CWWTP treats the ww; if 0: CCHP-WWT treats the ww


## Constants for the EV Charging demand
Optimize_EV_Load_On_CCHP = False # If true, percentage of EV Charging will be optimized
Percent_EV_Load_On_CCHP = 100 # Percentage of EV charging load carried by the CCHP set as a constant; only considered when Optimize_EV_Load_On_CCHP == False


Num_Engines = 32        # Must be updated if new engines are added
Num_Chillers = 16       # Must be updated if new chillers are added
Max_Buildings_per_Type = 1000 
Building_Min = 0
Supply_Min = 1
Vars_Plus_Output = 46  # Must be updated if new chillers, engines, or outputs are added; Must also update optimization initialization
    

Max_Site_GFA = 10**6 # 647497 
Min_GFA = Max_Site_GFA*0.1
Max_FAR = 5.0 # Assumed
Max_GFA = Max_FAR*Max_Site_GFA # Following the paper
Min_Total_Buildings = 1
Min_Solar = 15                    # Per SF Ordinance
Max_Solar = 60                     # Percentage of available solar roof area: from http://www.nrel.gov/docs/fy14osti/60593.pdf
# Min_WWT = 1
# Num_WWT = 2
Min_EV_Load_On_CCHP = 0 # Minimum portion of EV charging load handled by the CCHP
Max_EV_Load_On_CCHP = 100 # Maximum portion of EV charging load handled by the CCHP
...
# Create input arrays for electricity, heating, and cooling # All in kW
...
# Create input array for EV charging load # in MW
EV_Input = PEV_Count_Scale * np.genfromtxt('Hourly_EV.csv', float, delimiter=',', skip_header=1) # Demand array read from 2025 projected demands and scaled for the midyear
EV_Charger_Types = np.genfromtxt('Hourly_EV.csv', str, delimiter=',', max_rows=1)
assert list(EV_Charger_Types) == ['Home_L1', 'Home_L2', 'Public_DC', 'Public_L2', 'Work_L2'] # The order of the load profiles in Hourly_EV.csv must stay this way
...
# !!! Create the Low and High Sequence values that control mutation in optimization !! NOTE First elements of run_results must match this
Low_Seq = []
High_Seq = []
...
Low_Seq += [Min_Solar]
High_Seq += [Max_Solar]
if Optimize_EV_Load_On_CCHP == True: # If the percentage of EV charging load on CCHP is to be optimized
    Low_Seq += [Min_EV_Load_On_CCHP]
    High_Seq += [Max_EV_Load_On_CCHP]



def SupplyandDemandOptimization(Building_Var_Inputs, plot_profiles=False):    
    ...
    Comm_Solar_Var = Building_Var_Inputs[Num_Buildings+2]
    if Optimize_EV_Load_On_CCHP == True:
        EV_Load_Ratio = Building_Var_Inputs[Num_Buildings+3]
    else:
        EV_Load_Ratio = Percent_EV_Load_On_CCHP
    
    Comm_Solar_Type_Var = 2 ## ?? WHAT IS IT? Only the second solar type is used! NOT OPTIMIZING FOR SOLAR PANEL TYPE
    ...
    if np.sum(Building_Var_Inputs[:Num_Buildings]) == 0: ## TRIVIAL CASE AVOIDANCE
        ...
        Run_Result[0][Num_Buildings+3] = EV_Load_Ratio # i.e. element 24
    ...
    Diversifier_Peak = 1.0 # was 0.8 ## ??
    Aggregate_Demand = 0
    for i in range(Num_Buildings):
        j = i+1
        Aggregate_Demand += Diversifier_Peak*(Building_Vars[j]*Demand_Types[j][:,0:3]) ## MODIFIED for water demand+syntax shortened (columnstack replaced)
    
    Total_Electricity_Demand = np.sum(Aggregate_Demand[:,0]) # E demand from buildings
    Peak_Electricity_Demand = np.max(Aggregate_Demand[:,0]) # E demand from buildings
    
    
    
    
    # Add the proportional electricity demand for the EV charging infrastructure    
    Total_EV_Demand, EV_Charging_GHG, EV_Charging_LCC, Peak_EV_Demand, EV_Hourly_Charging_Load =\
        EV_Load_Calculator(Num_Buildings, Type, Diversifier_Peak, Building_Vars, Demand_Types,
...
#     Peak_EV_Demand = 0 # kW
#     
#     if EV_Load_Ratio != 0:     # Check if there's any EV charging before calculating its load
...
# =============================================================================
    ...
    # TES_Capex = 95*TES_Max/1000 * USD_2008_to_2021 # In 2019 USD # Averaged based on Table 8 from Cost for Sensible and other heat storage... @ D:\PhD\+Main Research Directory\W&WW+low-heat applications\+++ TES
...
#        Chiller_Hourly_Cooling_Results[i] = Hourly_Chiller_Result[3] ## UNUSED
    ...
    Total_Electric_Load_on_CCHP = np.sum(Engine_Demand[:,0])
    Peak_Electric_Load_on_CCHP = np.max(Engine_Demand[:,0])
...
# =============================================================================
    ...
    for i in range(Num_Buildings):
        j = i+1
        Comm_Solar_Area += Comm_Solar_Var/100*Solar_Roof_Area[j]*Building_Vars[j]



    # Calculate loads and subtract from total electrical demand; calculate costs and total solar capacity installed
    [Hourly_Solar_Generation, Capital_Solar_Cost] = Commercial_Solar_Types[Comm_Solar_Type_Var](np.array(range(8760)), UTC, Comm_Solar_Area, Tilt, Azimuth, Latitude, Longitude, Hourly_DNI, Hourly_DHI, Hourly_GHI, Hourly_Albedo, Hourly_Temperature, Hourly_Wind_Speed, Site_Altitude)[3:5]
...
#    GW_Efficiency = np.nan_to_num(Total_Treated_GW/Total_GW) ## Added the nan_to_number
    ...
    if CWWTP_Mode == 0:
        Capex = CCHP_Capex + np.max(Chiller_Costs) + Capital_Solar_Cost #+ TES_Capex #  + WWT_Capex_Total +  # MODIFIED: Solar added ##MODIFIED # Optimization objective
    # else:
        # Capex = CCHP_Capex + np.max(Chiller_Costs) + Capital_Solar_Cost + TES_Capex # MODIFIED: Solar added ##MODIFIED # Optimization objective
    Total_Capex = CCHP_Capex + np.max(Chiller_Costs) + Capital_Solar_Cost #+ TES_Capex # WWT_Capex_Total # MODIFIED: Solar added ##MODIFIED # For recording
    ## Added: sell price usage
    
    
    # Carbon emissions # ADDED/ MODIFIED 
    Construction_Carbon = (Capex / USD_2007_to_2021)/10**6 * Const_Carbon_per_Mil_Dollar # in metric tons CO2 eq # Optimization objective
    Total_Construction_Carbon = (Total_Capex / USD_2007_to_2021)/10**6 * Const_Carbon_per_Mil_Dollar # in metric tons CO2 eq # For recording
    
    Annual_Carbon_Emissions = np.sum(Carbon_Emissions)/MT_to_lbs # in metric tons CO2e # Optimization objective
    Total_Annual_Carbon_Emissions = np.sum(Carbon_Emissions)/MT_to_lbs + EV_Charging_GHG #+ WWT_GHG # in metric tons CO2e # For recording
    ...
    GHG = Construction_Carbon + Project_Life * Annual_Carbon_Emissions # ADDED/ MODIFIED # in metric tons CO2e
    Total_GHG = (Total_Construction_Carbon + Project_Life * Total_Annual_Carbon_Emissions) # ADDED/ MODIFIED # in metric tons CO2e
    ...
    for i in range(Project_Life):
        if CWWTP_Mode == 0:
            LCC += (CCHP_Opex)/(1+Discount_Rate)**i # WWT_Opex_Total # Optimization objective
        # else:
            # LCC += CCHP_Opex/(1+Discount_Rate)**i # Optimization objective
        LCC_Total += (CCHP_Opex + EV_Charging_LCC)/(1+Discount_Rate)**i # + WWT_Opex_Total # Total LCC for the record
    ...
    # Now the objectives
    Run_Result[0][Num_Buildings+7+Type_Max+10] = Overall_Efficiency # i.e. element 45
    Run_Result[0][Num_Buildings+5] = LCC_Total # $ # i.e. element 26
    Run_Result[0][Num_Buildings+6] = Total_GHG # Ton CO2e # i.e. element 27 # in $
    Run_Result[0][Num_Buildings+7] = LCC # old: Overall_CHP_Efficiency # i.e. element 28
    ...
    Run_Result[0][Num_Buildings+7+Type_Max+2] = Average_Height # Ft # i.e. element 37 # in ft!!
    Run_Result[0][Num_Buildings+7+Type_Max+3] = Total_GFA # m2 # i.e. element 38 # in m2
    
    ## Leftover from objectives! Out of laziness!!
    Run_Result[0][Num_Buildings+7+Type_Max+4] = GHG # Ton CO2e # i.e. element 39 # in metric tons CO2e
    
    # Now add the watch variables
    Run_Result[0][Num_Buildings+4] = Total_Electricity_Demand # i.e. element 25 # Electric demand from buildings 
    
    Run_Result[0][Num_Buildings+7+Type_Max+5] = Total_Electric_Load_on_CCHP # Old: Total_Energy_Demand # kWh # Electricity, heating and cooling #Old: Total_Excess_Electricity ## originally: Total_Capex # i.e. element 40
    Run_Result[0][Num_Buildings+7+Type_Max+6] = Peak_Electric_Load_on_CCHP # kW # Electric load from heating, cooling, and electricity on CCHP # element 41
    Run_Result[0][Num_Buildings+7+Type_Max+7] = Peak_Electricity_Demand # Electric demand from buildings #Old: Total_Excess_Heat ## originally: CCHP_Opex # i.e. element 42 # in L
    Run_Result[0][Num_Buildings+7+Type_Max+8] = Total_EV_Demand # Electric demand from EV charging # i.e. element 43
    Run_Result[0][Num_Buildings+7+Type_Max+9] = Peak_EV_Demand # Electric demand from EV charging # i.e. element 44
    ...
    if plot_profiles:
        ...
        Jan_1_hours = np.array(range(24))
        May_1_hours = Jan_1_hours + 120*24
        ...
        for day_name, hours in hours_dict.items():
            ...
            # Plot the combined plots for the 3 selected days
            # combined_EV_Buildings_Load =\
            #     EV_Hourly_Charging_Load[hours] +\
            #     Aggregate_Demand[hours,0]
            
            # nrm_combined_EV_Buildings_Load =\
            #     combined_EV_Buildings_Load / max(combined_EV_Buildings_Load)
            y_EV_nrm = EV_Hourly_Charging_Load[hours]/max(EV_Hourly_Charging_Load[hours])
            # y_Buildings_nrm = Aggregate_Demand[hours,0]/max(Aggregate_Demand[hours,0])
            # y_grid_E_price_nrm = Buy_Price[hours] / max(Buy_Price[hours])
            y_emissions_nrm = Grid_Emissions[hours] / max(Grid_Emissions[hours])
            ...
            plt.plot(x, y_EV_nrm, label='EV_Charging_Demand', linewidth=2, color='blue')
            # plt.plot(x, y_Buildings_nrm, label='Bldgs_Electricity_Demand', linewidth=2, color='red')
            # plt.plot(x, nrm_combined_EV_Buildings_Load,
            #          label='Bldgs_and_EV_Electricity_Demand', linewidth=2, color='red')
            # plt.plot(x, y_grid_E_price_nrm, label='Grid_Avg_Purchase_Price', linewidth=2, color='green')
            plt.plot(x, y_emissions_nrm, label='Grid_Avg_Emissions', linewidth=2, color='black')
            plt.legend()
            plt.xlabel(r'Hour')
            # plt.ylabel(r'Normalized_Electric_Demand | Grid_Price | Grid_Emissions')
            # plt.savefig('figures/Combined_Demand_Normalized_Plots_%s_%s.png'
            #             %(day_name,current_time), dpi=400, bbox_inches='tight')
            plt.ylabel(r'Normalized_Electric_Demand | Grid_Emissions')
            plt.savefig('figures/Combined_EV_Demand_Purchase_Price_Normalized_Plots_%s_%s.png'
                        %(day_name,current_time), dpi=400, bbox_inches='tight')
    ...
    if Total_GFA == 0: # Added for avoiding division by zero in the objectives (below) for trivial solutions
        Total_GFA = 0.00001
    return ((LCC/Total_GFA, GHG/Total_GFA, ),
...
if __name__ == "__main__":
    ...
    np.savetxt("Results.csv", TestRuns[1:], fmt='%f', delimiter=',')
